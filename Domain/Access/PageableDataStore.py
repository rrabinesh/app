...
# FastAPI equivalent of the PageInfo class
...
# FastAPI equivalent of the PageableData class
class PageableData(BaseModel):
    Results: List[dict]
    PageInfo: PageInfo

# FastAPI endpoint to implement the GetList functionality
@app.post("/get-list/", response_model=PageableData)
def get_list(pageNumber: int = None, pageSize: int = None, sortField: str = None, sortDirection: str = None):
    # Replace the implementation with your actual query logic
    # Here, we'll just return a simple example data for demonstration purposes
    example_data = [
        {"id": 1, "name": "John"},
        {"id": 2, "name": "Jane"},
        {"id": 3, "name": "Michael"},
    ...
    # Calculate total results (replace this with the actual total result count from your query)
    total_results = len(example_data)

    # Implement sorting and pagination here (replace example_data with actual query results)
    sorted_data = sorted(example_data, key=lambda x: x.get(sortField, None), reverse=(sortDirection == "desc"))
    paginated_data = sorted_data[(pageNumber - 1) * pageSize: pageNumber * pageSize]

    # Create PageInfo object
    page_info = PageInfo(
        Total=total_results,
        TotalPages=(total_results // pageSize) + (1 if total_results % pageSize > 0 else 0),
        PageSize=pageSize,
        CurrentPage=pageNumber,
        ResultsStart=((pageNumber - 1) * pageSize) + 1,
        ResultsEnd=min(pageNumber * pageSize, total_results)
    )

    return PageableData(Results=paginated_data, PageInfo=page_info)
