...
@app.get("/reminders/user/{user_id}", response_model=List[Reminder])
def get_reminders_by_user(user_id: str):
    return [reminder for reminder in reminders_db if reminder.UserID == user_id]

@app.get("/reminders/refnumber/{refnumber}", response_model=List[Reminder])
def get_reminders_by_refnumber(refnumber: str):
    return [reminder for reminder in reminders_db if reminder.RefNumber == refnumber]

@app.delete("/reminders/{reminder_id}")
def delete_reminder(reminder_id: int):
    reminders_db[:] = [reminder for reminder in reminders_db if reminder.ReminderID != reminder_id]

@app.delete("/reminders/user/{user_id}")
def delete_reminders_by_user(user_id: str):
    reminders_db[:] = [reminder for reminder in reminders_db if reminder.UserID != user_id]

@app.delete("/reminders/refnumber/{refnumber}")
def delete_reminders_by_refnumber(refnumber: str):
    reminders_db[:] = [reminder for reminder in reminders_db if reminder.RefNumber != refnumber]
