...
class PeakTimes(BaseModel):
    ...
    def is_within_peak(self, time: Time) -> bool:
        return self.start.hour <= time.hour < self.end.hour
...
class ILayoutTimezone:
    timezone: str


class IDailyScheduleLayout:
    uses_daily_layouts: bool


class IScheduleLayout(ILayoutTimezone, IDailyScheduleLayout):

    def get_layout(self, layout_date: Date, hide_blocked_periods: bool = False) -> List[SchedulePeriod]:
        pass

    def get_period(self, date: Date) -> Optional[SchedulePeriod]:
        pass

    def get_slot_count(self, start_date: Date, end_date: Date) -> int:
        pass

    def change_peak_times(self, peak_times: PeakTimes) -> None:
        pass

    def remove_peak_times(self) -> None:
        pass

    def fits_to_hours(self) -> bool:
        pass
...
class ILayoutCreation(ILayoutTimezone, IDailyScheduleLayout):

    def append_period(self, start_time: Time, end_time: Time, label: Optional[str] = None, day_of_week: Optional[DayOfWeek] = None) -> None:
        pass

    def append_blocked_period(self, start_time: Time, end_time: Time, label: Optional[str] = None, day_of_week: Optional[DayOfWeek] = None) -> None:
        pass

    def get_slots(self, day_of_week: Optional[DayOfWeek] = None) -> List[LayoutPeriod]:
        pass
...
class ScheduleLayout(IScheduleLayout, ILayoutCreation):

    def __init__(self, target_timezone: Optional[str] = None):
        self.target_timezone = target_timezone or Configuration.Instance().get_default_timezone()
        self._periods = []
        self.layout_timezone = None
        self.cached = False
        self.cached_periods = {}
        self.using_daily_layouts = False
        self.start_times = {}

    def get_slots(self, day_of_week: Optional[DayOfWeek] = None) -> List[LayoutPeriod]:
        if day_of_week is None:
            if self.using_daily_layouts:
                raise Exception('ScheduleLayout->get_slots() day_of_week required when using daily layouts')
            periods = self._periods
        else:
            if not self.using_daily_layouts:
                raise Exception('ScheduleLayout->get_slots() day_of_week cannot be provided when using single layout')
            periods = self._periods[day_of_week.value]
        # Sorting logic here
        return periods

    def append_period(self, start_time: Time, end_time: Time, label: Optional[str] = None, day_of_week: Optional[DayOfWeek] = None) -> None:
        self._append_generic_period(start_time, end_time, PeriodTypes.RESERVABLE, label, day_of_week)

    def append_blocked_period(self, start_time: Time, end_time: Time, label: Optional[str] = None, day_of_week: Optional[DayOfWeek] = None) -> None:
        self._append_generic_period(start_time, end_time, PeriodTypes.NONRESERVABLE, label, day_of_week)

    def _append_generic_period(self, start_time: Time, end_time: Time, period_type: int, label: Optional[str] = None, day_of_week: Optional[DayOfWeek] = None):
        # Append logic here
        pass
