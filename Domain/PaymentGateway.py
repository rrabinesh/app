...
#             Log::Error(
#                 'PayPal Checkout/Orders error details. Data: %s, CartId/invoice number: %s, Total: %s, Error: %s',
#                 $body,
#                 $cart->Id(),
#                 $cart->Total(),
#                 $exception
...
#             $err = $body['error'];
#             Log::Debug(
#                 'Stripe charge failed. http status %s, type %s, code %s, param %s, message %s',
#                 $ex->getHttpStatus(),
#                 $err['type'],
#                 $err['code'],
#                 $err['param'],
...
#                 $refund->status,
#                 $refund->id,
#                 $currency->FromStripe($refund->amount),
#                 $currency->FromStripe($refund->amount),
#                 $currency->FromStripe($refund->balance_transaction->fee),
#                 null,
#                 Date::Now(),
#                 $refund->created,
#                 json_encode($refund)
...
#             $err = $body['error'];
#             Log::Debug(
#                 'Stripe refund  failed. http status %s, type %s, code %s, param %s, message %s',
#                 $ex->getHttpStatus(),
#                 $err['type'],
#                 $err['code'],
#                 $err['param'],
...
class IPaymentGateway:
    ...
    def settings(self) -> List[PaymentGatewaySetting]:
        pass

class IPaymentTransactionLogger:
    def log_payment(self, **kwargs):
        pass

    def log_refund(self, **kwargs):
        pass

class PaymentTransactionLogger(IPaymentTransactionLogger):
    def log_payment(self, **kwargs):
...
class PayPalGateway(IPaymentGateway):
    ...
    def __init__(self, enabled: bool, client_id: str, secret: str, environment: str):
    ...
    @classmethod
    def create(cls, client_id: str, secret: str, environment: str) -> 'PayPalGateway':
        enabled = bool(client_id) and bool(secret) and bool(environment)
        return cls(enabled, client_id, secret, environment)
    ...
    def settings(self) -> List[PaymentGatewaySetting]:
        return [
            PaymentGatewaySetting(name=self.CLIENT_ID, value=self.client_id),
            PaymentGatewaySetting(name=self.SECRET, value=self.secret),
            PaymentGatewaySetting(name=self.ENVIRONMENT, value=self.environment),
        ]

    # Implement other methods such as create_payment, execute_payment, refund, etc.

class StripeGateway(IPaymentGateway):
    PUBLISHABLE_KEY = 'publishable_key'
    SECRET_KEY = 'secret_key'

    def __init__(self, enabled: bool, publishable_key: str, secret_key: str):
        self.enabled = enabled
        if enabled:
    ...
    @classmethod
    def create(cls, publishable_key: str, secret_key: str) -> 'StripeGateway':
        enabled = bool(publishable_key) and bool(secret_key)
        return cls(enabled, publishable_key, secret_key)
    ...
    def settings(self) -> List[PaymentGatewaySetting]:
        return [
            PaymentGatewaySetting(name=self.PUBLISHABLE_KEY, value=self.publishable_key),
            PaymentGatewaySetting(name=self.SECRET_KEY, value=self.secret_key),
        ]

    # Implement other methods such as charge, refund, etc.
