...
class UserPreferences(BaseModel):
    # Define UserPreferences fields here if needed
    pass

class UserGroup(BaseModel):
    GroupId: int
    GroupName: str
    AdminGroupId: Optional[int] = None
    IsGroupAdmin: bool = False
    IsApplicationAdmin: bool = False
    IsResourceAdmin: bool = False
    IsScheduleAdmin: bool = False
...
class EmailPreferences(BaseModel):
    ...
    def add(self, eventCategory: str, eventType: str):
        key = f"{eventCategory}|{eventType}"
        self.preferences[key] = True

    def delete(self, eventCategory: str, eventType: str):
        key = f"{eventCategory}|{eventType}"
        if key in self.preferences:
            del self.preferences[key]

    def exists(self, eventCategory: str, eventType: str):
        key = f"{eventCategory}|{eventType}"
        return key in self.preferences

    def add_preference(self, event: dict):
        if not self.exists(event["EventCategory"], event["EventType"]):
            self.add(event["EventCategory"], event["EventType"])
            self._added.append(event)

    def remove_preference(self, event: dict):
        if self.exists(event["EventCategory"], event["EventType"]):
            self.delete(event["EventCategory"], event["EventType"])
...
class User(BaseModel):
    ...
    addedGroups: List[UserGroup] = []
    removedGroups: List[UserGroup] = []
    groupsICanAdminister: List[UserGroup] = []
    isCalendarSubscriptionAllowed: bool = False
    originalCredits: float = 0.0
    isGroupAdmin: bool = False
    isApplicationAdmin: bool = False
    isResourceAdmin: bool = False
    isScheduleAdmin: bool = False
    publicId: Optional[str] = None
    creditsNote: Optional[str] = None
    permissionsChanged: bool = False
    removedPermissions: List[int] = []
    addedPermissions: List[int] = []
    removedViewPermissions: List[int] = []
    addedViewPermissions: List[int] = []
    allowedResourceIds: List[int] = []
    viewableResourceIds: List[int] = []
    encryptedPassword: Optional[str] = None
    passwordSalt: Optional[str] = None
    attributes: dict = {}
    attributesChanged: bool = False
