...
class BlackoutResource(BaseModel):
    id: int
    name: str
    scheduleId: int
    adminGroupId: int
...
class RepeatConfiguration(BaseModel):
    # Define the fields here for RepeatConfiguration (not provided in the PHP code)
    pass
class RepeatNone(BaseModel):
    # Define the fields here for RepeatNone (not provided in the PHP code)
    pass
class BlackoutSeries(BaseModel):
    seriesId: int
    ownerId: int
    resourceIds: List[int]
    resources: List[BlackoutResource]
    blackouts: List[Blackout]
    blackoutIteration: int
    title: str
    blackoutDate: DateRange
    repeatOptions: RepeatNone
    repeatConfiguration: RepeatConfiguration
    isNew: bool
    currentBlackoutInstanceId: int
    customRepeatDates: List[date]

    # Define the methods here as needed (not provided in the PHP code)
    # Note: The methods and properties in PHP code require a custom implementation
