...
from services import ScheduleService, ResourceService, ReservationService 
...
class SchedulePage:
    ...
    def __init__(
        ...
        request: Request,
        db=Depends(get_db),
        presenter: SchedulePresenter = Depends(),
        ...
        self.schedule_id = request.query_params.get('schedule_id')
        # other attributes
        
        self.presenter.get_schedule(db, request.user)
        
    async def get(self):
        schedule = await self.schedule_service.get(self.schedule_id)
        resources = await self.resource_service.get_for_schedule(schedule.id)
    ...
    async def get_layout(self, date: date):
        return await self.presenter.get_layout(self.request.user, date)
        
    async def load_reservations(self):
        reservations = await self.presenter.load_reservations(
            self.request.user,
            self.schedule_id,
            self.date_filter
        )
