...
class Response:
    def __init__(self, status_code, text, headers, json_opts):
...
class Request:
    ...
    @classmethod
    def json_opts(cls, assoc=False, depth=512, options=0):
        cls.json_opts = {
            'indent': None,
            'separators': (',', ':'),
            'ensure_ascii': True,
    ...
    @classmethod
    def verify_peer(cls, enabled):
        cls.verify_peer = enabled

    @classmethod
    def verify_host(cls, enabled):
        cls.verify_host = enabled

    @classmethod
    def timeout(cls, seconds):
        cls.socket_timeout = seconds
    ...
    @classmethod
    def default_header(cls, name, value):
        cls.default_headers[name] = value

    @classmethod
    def clear_default_headers(cls):
        cls.default_headers = {}

    @classmethod
    def curl_opts(cls, options):
        cls.curl_opts.update(options)

    @classmethod
    def curl_opt(cls, name, value):
        cls.curl_opts[name] = value

    @classmethod
    def clear_curl_opts(cls):
        cls.curl_opts = {}

    @classmethod
    def set_mashape_key(cls, key):
        cls.default_header('X-Mashape-Key', key)
    ...
    @classmethod
    def cookie_file(cls, cookie_file):
        cls.cookie_file = cookie_file

    @classmethod
    def auth(cls, username='', password='', method=requests.auth.HTTPBasicAuth):
        cls.auth = requests.auth.HTTPBasicAuth(username, password)

    @classmethod
    def proxy(cls, address, port=1080, tunnel=False, type_=requests.utils.DEFAULT_CA_BUNDLE_PATH):
        cls.proxy = {'http': f'{address}:{port}', 'https': f'{address}:{port}', 'tunnel': tunnel, 'type': type_}

    @classmethod
    def proxy_auth(cls, username='', password='', method=requests.auth.HTTPBasicAuth):
        cls.proxy_auth = requests.auth.HTTPBasicAuth(username, password)

    @classmethod
    def build_http_curl_query(cls, data, parent=False):
        result = {}
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if isinstance(value, dict) or isinstance(value, list):
                    result.update(cls.build_http_curl_query(value, new_key))
                else:
                    result[new_key] = value
    ...
    @classmethod
    def send(cls, method, url, body=None, headers=None, username=None, password=None):
        json_opts = cls.json_opts
        ...
        if method != Method.GET:
            ...
            # ... Set other request-related options based on `kwargs` ...
            ...
            if '?' in url:
                ...
                url += '?'
            url += requests.utils.unquote(requests.utils.urlencode(cls.build_http_curl_query(body)))

            # ... Set other request-related options for GET method ...

            response = requests.get(url)

        return Response(response.status_code, response.text, response.headers, json_opts)

    # ... Other methods as in the original Unirest\Request class ...
