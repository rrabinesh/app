...
class Date:
    def __init__(self, timestring=None, timezone=None):
        self.timezone = timezone if timezone else datetime.now().astimezone().tzinfo
        self.date = datetime.strptime(timestring, "%Y-%m-%d %H:%M:%S") if timestring else datetime.now()

    def __str__(self):
        return self.date.strftime("%Y-%m-%d %H:%M:%S") + " " + str(self.timezone)

    def to_timezone(self, timezone):
        if self.timezone == timezone:
            return self.copy()

        return Date(self.date.astimezone(timezone), timezone)

    def copy(self):
        return Date(self.date.strftime("%Y-%m-%d %H:%M:%S"), self.timezone)

    def add_days(self, days):
        new_date = self.date + timedelta(days=days)
        return Date(new_date.strftime("%Y-%m-%d %H:%M:%S"), self.timezone)

    def subtract_minutes(self, minutes):
        new_date = self.date - timedelta(minutes=minutes)
        return Date(new_date.strftime("%Y-%m-%d %H:%M:%S"), self.timezone)
...
@app.get("/date/is_weekday")
def check_weekday(date_str: str):
    d = Date(date_str)
    return {"is_weekday": d.is_weekday()}

@app.get("/date/is_weekend")
def check_weekend(date_str: str):
    d = Date(date_str)
    return {"is_weekend": d.is_weekend()}
